...
void main() {
    vec3 color = ourColor * brightness;
...
glm::vec3 getStickerColor(int face) {
    ...
    case 1: return { 1.0f, 0.5f, 0.0f };
    case 2: return { 1.0f, 1.0f, 1.0f };
    case 3: return { 1.0f, 1.0f, 0.0f };
    case 4: return { 0.0f, 0.7f, 0.0f };
    case 5: return { 0.0f, 0.3f, 1.0f };
...
bool cubieInSelectedLayer(const Cubie& c) {
    if (anim.active) {
        return cubieInLayer(c, anim.axis, anim.layer);
    }

    return cubieInLayer(c, selectedAxis, currentLayerValue());
}
...
void processInput(GLFWwindow* window) {
    ...
    if (glfwGetKey(window, GLFW_KEY_X) == GLFW_PRESS && !qPressed) {
        selectedAxis = Axis::X;
        startMove(Axis::X, currentLayerValue(), dir);
        qPressed = true;
    }
    if (glfwGetKey(window, GLFW_KEY_X) == GLFW_RELEASE) qPressed = false;

    if (glfwGetKey(window, GLFW_KEY_Y) == GLFW_PRESS && !wPressed) {
        selectedAxis = Axis::Y;
        startMove(Axis::Y, currentLayerValue(), dir);
        wPressed = true;
    }
    if (glfwGetKey(window, GLFW_KEY_Y) == GLFW_RELEASE) wPressed = false;

    if (glfwGetKey(window, GLFW_KEY_Z) == GLFW_PRESS && !ePressed) {
        selectedAxis = Axis::Z;
        startMove(Axis::Z, currentLayerValue(), dir);
        ePressed = true;
    }
    if (glfwGetKey(window, GLFW_KEY_Z) == GLFW_RELEASE) ePressed = false;
...
void drawCubie(const Cubie& c, const glm::mat4& view, const glm::mat4& projection) {
    ...
    if (cubieInSelectedLayer(c)) {
        brightness = 1.35f;
        scaleValue = 1.05f;
    }

    model = model * glm::scale(glm::mat4(1.0f), glm::vec3(scaleValue));
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        string title = "Rubik Cube OpenGL | Layer: " + to_string(currentLayerValue()) +
            " | Axis: " + axisToString(selectedAxis) +
            " | 1 / 2 / 3 layer | X / Y / Z rotate | Shift reverse| R reset | LMB drag camera";
